...
class TherapeuticProfileScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<TherapeuticProfileScreen> createState() => _TherapeuticProfileScreenState();
}

class _TherapeuticProfileScreenState extends ConsumerState<TherapeuticProfileScreen> {
  ...
  Future<void> _pickFile() async {
    ...
    if (result != null && result.files.single.path != null) {
      // Simular subida
      final fileName = await ref.read(perfilServiceProvider).cargarDiagnostico(result.files.single.path);
      setState(() {
      ...
      body: perfilAsync.when(
        data: (perfil) {
          if (!_dataInitialized) {
            ...
            _aversiveStimuli = Map.from(perfil.estimulosAversivos);
          ...
          Center(
            child: Column(
              children: [
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(color: const Color(0xFFB8D6B2), width: 4),
                        image: const DecorationImage(
                          image: NetworkImage('https://i.pravatar.cc/300?img=11'), // mock niño
                          fit: BoxFit.cover,
                        ),
                      ),
                    ),
                    Container(
                      ...
                      child: IconButton(
                        icon: const Icon(Icons.edit, size: 16, color: Color(0xFF1E1B16)),
                        onPressed: () {},
                ...
                _buildTextField("Edad", _ageController, keyboardType: TextInputType.number),
              ],
            ),
          ),
        ],
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              ...
              ElevatedButton.icon(
                ...
                label: const Text("Añadir", style: TextStyle(color: Color(0xFF1E1B16))),
          ...
          GridView.count(
            ...
            crossAxisSpacing: 16,
            mainAxisSpacing: 16,
            childAspectRatio: 1, // To make them square-ish based on height spec 128
            children: [
              InterestCard(
                label: "Dinosaurios",
                imageUrl: "https://images.unsplash.com/photo-1596489370607-bb1b590e82aa?q=80&w=300",
                isSelected: _selectedInterests.contains("Dinosaurios"),
                onTap: () {
                  setState(() => _selectedInterests.contains("Dinosaurios") ? _selectedInterests.remove("Dinosaurios") : _selectedInterests.add("Dinosaurios"));
                },
              ),
              InterestCard(
                label: "Trenes",
                imageUrl: "https://images.unsplash.com/photo-1517524285303-d6fc683dddf8?q=80&w=300",
                isSelected: _selectedInterests.contains("Trenes"),
                onTap: () {
                  setState(() => _selectedInterests.contains("Trenes") ? _selectedInterests.remove("Trenes") : _selectedInterests.add("Trenes"));
                },
              ),
              InterestCard(
                label: "Lego",
                imageUrl: "https://images.unsplash.com/photo-1585366119957-e9730b6d0f60?q=80&w=300",
                isSelected: _selectedInterests.contains("Lego"),
                onTap: () {
                  setState(() => _selectedInterests.contains("Lego") ? _selectedInterests.remove("Lego") : _selectedInterests.add("Lego"));
                },
              )
            ],
          ),
          ...
          Wrap(
            ...
            children: [
              SizedBox(
                width: MediaQuery.of(context).size.width < 768 ? double.infinity : (MediaQuery.of(context).size.width/2 - 100),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    Wrap(
                      ...
                      runSpacing: 8,
                      children: [
                        TagChip(label: "Manzana", isSelected: _selectedInterests.contains("Manzana"), onTap: () => setState(() => _selectedInterests.contains("Manzana") ? _selectedInterests.remove("Manzana") : _selectedInterests.add("Manzana"))),
                        TagChip(label: "Galletas", isSelected: _selectedInterests.contains("Galletas"), onTap: () => setState(() => _selectedInterests.contains("Galletas") ? _selectedInterests.remove("Galletas") : _selectedInterests.add("Galletas"))),
                      ],
                    ),
                  ],
                ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      runSpacing: 8,
                      children: [
                        TagChip(label: "Pelota ahulada", isSelected: _selectedInterests.contains("Pelota ahulada"), onTap: () => setState(() => _selectedInterests.contains("Pelota ahulada") ? _selectedInterests.remove("Pelota ahulada") : _selectedInterests.add("Pelota ahulada"))),
                      ],
                    ),
                  ],
                ),
    ...
    return BentoCard(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          _buildAversiveCategory("RUIDO", ["Licuadora", "Gritos", "Aspiradora", "Música alta"]),
          const SizedBox(height: 20),
          _buildAversiveCategory("COLORES", ["Rojo chillón", "Amarillo brillante", "Neón"]),
          const SizedBox(height: 20),
          _buildAversiveCategory("LUGARES", ["Multitudes", "Cuartos blancos", "Espacios reducidos"]),
        ],
      ),
    );
  }
      ...
      children: [
        ...
        Wrap(
          ...
          children: options.map((opt) {
            final isSelected = _aversiveStimuli[categoryName]?.contains(opt) ?? false;
            return TagChip(
              label: opt,
  ...
  Widget _buildUploadDiagnosisCard() {
    return BentoCard(
      ...
      border: Border.all(color: const Color(0xFFB8D6B2).withOpacity(0.3), width: 2),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
                 ...
                 Expanded(
                   child: Column(
                     ...
                     children: [
                       ...
                       const Text("Subido hoy", style: TextStyle(fontSize: 12, color: Colors.grey)),
                     ],
                   ),
                 ),
                 IconButton(
            ...
            InkWell(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: const Color(0xFFB8D6B2), style: BorderStyle.solid), 
                  color: Colors.white.withOpacity(0.5),
                ),
                child: Column(
                  children: [
                    const Icon(Icons.cloud_upload, size: 36, color: Color(0xFFB8D6B2)),
                    const SizedBox(height: 12),
                    const Text(
                      "Seleccionar archivo",
                      style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF4A624D)),
                    ),
                    const SizedBox(height: 4),
                    Text(
                      "PDF, JPG o PNG (Max 10MB)",
                      style: TextStyle(fontSize: 11, color: const Color(0xFF58423B).withOpacity(0.5)),
                    ),
                  ],
                ),
              ),
